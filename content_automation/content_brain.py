...
Plan the next Instagram post following the brand guide above. Rules:
- post_text is displayed ON the image -- bold, punchy, max 10 words, MUST be in Albanian.
- caption is the Instagram caption -- Albanian, up to 120 words, include hashtags, end with "Behu me i mprehte me ne."
...
def generate_image_prompts(theme: str, post_text: str, ref_images: list[Path], handle: str = "sharp_group") -> list[str]:
    """
...
Your task: decide what the BEST possible visual concept is for this specific theme and text, then write one detailed Fal.ai (Flux model) prompt for it.

You have full creative freedom to choose the visual approach — it could be:
- A 3D symbolic object that represents the concept (chess piece, arrow, target, megaphone, person on a phone, hands on a laptop, etc.)
- A graphic / geometric composition
- A human element in a business context
- A metaphorical scene
- Anything else that best communicates the theme visually

Choose whatever will make the strongest, most on-brand image for this specific post. Think like a creative director, not a template.
