...
REEL_SYSTEM_PROMPT = (
    ...
    "You will receive a research brief as JSON. Write a punchy ~40-second reel script and return ONLY a valid JSON object "
    "with exactly these fields:\n"
    "  title        — string: a short catchy title for the reel\n"
    "  runtime      — string: estimated runtime e.g. '~42 seconds'\n"
...
VISUAL_SYSTEM_PROMPT = (
    ...
    "Your job: select the 5-6 most crucial moments across the entire script that need a visual — "
    "moments where an image will have the strongest impact on the viewer. "
    "Do NOT generate one image per segment. Instead, identify the key turning points, the hook, "
    "the main revelation, and the call to action — whichever moments carry the most weight. "
    "Each image covers roughly 7-12 seconds of video. One main idea per frame — keep it simple and punchy. "
    "Return ONLY a valid JSON array of 5-6 objects, each with exactly these fields:\n"
    ...
    "  image_prompt  — string: a full ready-to-paste image generation prompt — describe the scene in vivid detail "
