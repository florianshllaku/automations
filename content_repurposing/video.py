...
def generate_video(visuals: list[dict], audio_path: str, srt_path: str, title_slug: str, output_path: str = None) -> str | None:
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".txt", delete=False, encoding="utf-8") as f:
        ...
        for i, (v, dur) in enumerate(zip(valid, durations)):
            img_path = str(Path(v["image_path"]).resolve()).replace("\\", "/")
            f.write(f"file '{img_path}'\n")
            f.write(f"duration {dur:.3f}\n")
            log(f"[DEBUG] Concat entry {i+1}: {img_path} @ {dur:.3f}s", "DEBUG")
        # last file repeated without duration (ffmpeg requirement)
        last_path = str(Path(valid[-1]["image_path"]).resolve()).replace("\\", "/")
    ...
    try:
        ...
        log("ffmpeg timed out after 300s", "ERROR")
        return None
